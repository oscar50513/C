...
int Find_Legal_Moves( int color )
{
    ...
    int legal_count = 0;
    // 初始化可以下的位置盤面
    memset(Legal_Moves,0,sizeof(int)*Board_Size*Board_Size);
...
int getstable(int myturn){
    ...
    int i,j;
    for(i=0;i<4;i++){
        if (Now_Board[cind1[i]][cind2[i]] == Stones[myturn]){
            stop[i] = 1;
            stable[0] += 2;
            for(j=1;j<Board_Size;j++){
                if(Now_Board[cind1[i]+inc1[i]*j][cind2[i]+inc2[i]*j] != Stones[myturn])
                    break;
                else{
                    stop[i] = j + 1;
                    stable[1] += 1;}}}}
    for(i=0;i<4;i++){
        if (Now_Board[cind1[i]][cind2[i]] == Stones[myturn]){
            for(j=1;j<Board_Size-stop[i-1];j++){
                if(Now_Board[cind1[i]+inc1[i]*j][cind2[i]+inc2[i]*j] != Stones[myturn])
                    break;
                else{
                    stable[1] += 1;}}}}
